...
@Component({
  selector: 'app-tubd-summative-task',
  templateUrl: './tubd-summative-task.component.html',
  styleUrls: ['./tubd-summative-task.component.scss']
})
export class TubdSummativeTaskComponent implements OnInit {
  categoryId: any;
  editor;
  modalRef: BsModalRef;
  ...
  IsDigitalToolUsed: boolean = false;
  ...
  createTask(isDigitalToolUsed: any){

    console.log('Increate task');
    this.performanceService.addNewTask(
                              this.NewUnitTitle,
                              this.NewUnitDateRange && this.NewUnitDateRange[0]? (new Date(this.NewUnitDateRange[0])).toDateString():'',
                              this.NewUnitDateRange && this.NewUnitDateRange[1]? (new Date(this.NewUnitDateRange[1])).toDateString():'',
                              this.unitId,
                              this.description,
                              this.courseId,
                              this.assessmentId,
...
this.selectedTask = task;
  // if(type=='add'){
    ...
    this.IsDigitalToolUsed = task.is_digital_tool_used;
    ...
    this.courseService.getCourse(this.courseId)
      .subscribe((response) => {
        ...
        this.courseDetails.content_categories.forEach(element => {
          ...
          if (element.type ==='digital_strategies'){
            console.log('Found Digital Stategies');
            this.digitalStrategyId = element.id;
            this.digitalStrategies = element
            console.log(this.digitalStrategies);
          }
        })
      }, (error) => {
        console.warn(error);
